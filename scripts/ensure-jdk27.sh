...
# in the caller's shell. Used by run.sh to launch the ChucK-Java IDE.
#
# Resolution order:
...
_chuck_jdk27=$(_chuck_jdk27_home)
if [ -n "$_chuck_jdk27" ]; then
  export JAVA_HOME="$_chuck_jdk27"
  JAVA_EXEC="$JAVA_HOME/bin/java"
elif _chuck_system_java_is_27; then
  JAVA_EXEC="java"
else
