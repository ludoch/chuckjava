...
public class SequencerApp extends Application {
  ...
  private synchronized void loadEngine(boolean force) {
    ...
    // Re-register sequencer panel globals
    if (sequencerPanel != null) {
      sequencerPanel.registerGlobals();
    }

    if (selectedCkFile != null) {
      vm.add(selectedCkFile.getAbsolutePath());
    } else {
      java.io.File file = org.chuck.core.ChuckConfig.resolveFile(DEFAULT_ENGINE);
      if (file.exists()) {
        vm.add(file.getAbsolutePath());
      } else {
        // Fallback to classpath resource
        try (java.io.InputStream is =
            SequencerApp.class.getResourceAsStream("/" + DEFAULT_ENGINE)) {
          if (is == null) {
            String fallback = DEFAULT_ENGINE.replace("chuck-core/", "");
            try (java.io.InputStream is2 = SequencerApp.class.getResourceAsStream("/" + fallback)) {
              if (is2 != null) {
                String code =
                    new String(is2.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
                vm.run(code, fallback);
                return;
              }
            }
          } else {
            String code = new String(is.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
            vm.run(code, DEFAULT_ENGINE);
            return;
          }
          System.err.println("Could not find " + DEFAULT_ENGINE + " in filesystem or classpath");
  ...
  public void start(Stage primaryStage) {
    ...
    stopBtn.setOnAction(
        e -> {
          vm.clear();
          vm.setGlobalInt("seq_current_step", -1L);
        });
