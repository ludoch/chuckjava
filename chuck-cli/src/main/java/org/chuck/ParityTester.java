...
/**
 * ParityTester: A Java-based regression suite that compares ChucK-Java output against native C++
 * ChucK.
 */
public class ParityTester {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (String ck : CK_FILES) {
      ...
      if (base.equals("06_event_broadcast")) {
        compareWavs(
            new File("comparison/native/06_event_broadcast_440.wav"),
            new File("comparison/java/06_event_broadcast_440.wav"));
        compareWavs(
            new File("comparison/native/06_event_broadcast_880.wav"),
            new File("comparison/java/06_event_broadcast_880.wav"));
      } else if (base.equals("07_shred_spork")) {
        for (int i = 0; i < 5; i++) {
          compareWavs(
              new File("comparison/native/07_shred_spork_" + i + ".wav"),
              new File("comparison/java/07_shred_spork_" + i + ".wav"));
        }
      }
  ...
  private static void runJava(String ckFile, String wavFile) {
    ...
    cli.run(
        new String[] {"--silent", ckFile + ":" + wavFile, "--halt", "--timeout:10", "--verbose:0"});
  }
  ...
  private static class ComparisonResult {
    ...
    ComparisonResult(double rms, String status) {
      this.rms = rms;
      this.status = status;
    }
  }
