...
-   **🔊 Audio DSP & Wave Parity Safeguards:**
    -   **Automated Wave Parity Assertion:** Every core playhead, voice mixer, or DSP engine modification MUST pass the direct sequential comparative check inside [DigitalAudioFidelityTest.java](deluge/src/test/java/org/chuck/deluge/DigitalAudioFidelityTest.java). We sequentially assert wave frame shape linear alignment (scale factors matching target track gain levels), infinite decay energy curves, and organic DC offset tolerances ($\le 0.01$).
