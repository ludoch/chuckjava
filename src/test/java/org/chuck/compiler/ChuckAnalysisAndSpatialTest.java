...
/**
 * Tests for analysis / spatial audio features: vec2/vec3/vec4 field accessors and arithmetic,
 * complex/polar arithmetic, operator overloading (comparison), ZCR, MFCC, SFM, Kurtosis, and FM
 * instrument variants.
 */
public class ChuckAnalysisAndSpatialTest {
  ...
  public void testFMInstrumentsInstantiate() {
    int sr = 44100;
    Wurley w = new Wurley(sr);
    w.noteOn(0.8f);
    w.setFreq(440.0);
    w.tick(0);
    BeeThree b = new BeeThree(sr);
    b.noteOn(0.8f);
    b.setFreq(440.0);
    b.tick(0);
    HevyMetl h = new HevyMetl(sr);
    h.noteOn(0.8f);
    h.setFreq(440.0);
    h.tick(0);
    PercFlut p = new PercFlut(sr);
    p.noteOn(0.8f);
    p.setFreq(440.0);
    p.tick(0);
    TubeBell t = new TubeBell(sr);
    t.noteOn(0.8f);
    t.setFreq(440.0);
    t.tick(0);
    FMVoices f = new FMVoices(sr);
    f.noteOn(0.8f);
    f.setFreq(440.0);
    f.tick(0);
