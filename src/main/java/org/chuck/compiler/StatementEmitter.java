    ...
    public void emitStatement(ChuckAST.Stmt stmt, ChuckCode code) {
        ...
        switch (stmt) {
            case ChuckAST.ExpStmt s -> {
                ...
                if (s.exp() instanceof ChuckAST.IdExp ide && parent.getLocalScopes().size() <= 1 && parent.getCurrentClass() == null) {
                    String nm = ide.name();
                    boolean knownName = parent.getVarTypeByName(nm) != null
                            || parent.getUserClassRegistry().containsKey(nm) || parent.isKnownUGenType(nm)
            ...
            case ChuckAST.DeclStmt s -> {
                ...
                if (isUserClass && s.callArgs() instanceof ChuckAST.CallExp call) {
                    if (parent.isInPreCtor()) {
                        ...
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushField(parent.getBaseType(s.type()), s.name(), 0, s.isReference(), false, parent.getUserClassRegistry()));
                    } else if (useGlobal) {
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushGlobal(parent.getBaseType(s.type()), s.name(), 0, s.isReference(), false, parent.getUserClassRegistry()));
                    } else {
                        ...
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushLocal(s.type(), offset, 0, s.isReference(), false, parent.getUserClassRegistry()));
                    }
                    code.addInstruction(new StackInstrs.Dup());
                    ...
                    if (parent.isInPreCtor()) {
                        code.addInstruction(new StackInstrs.PushThis());
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushField(parent.getBaseType(s.type()), s.name(), argCount, s.isReference(), isArrayDecl, parent.getUserClassRegistry()));
                        code.addInstruction(new StackInstrs.Pop());
                    } else if (useGlobal) {
                        parent.getGlobalVarTypes().put(s.name(), s.type());
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushGlobal(parent.getBaseType(s.type()), s.name(), argCount, s.isReference(), isArrayDecl, parent.getUserClassRegistry()));
                        code.addInstruction(new StackInstrs.Pop());
                    } else {
                        Map<String, Integer> scope = parent.getLocalScopes().peek();
                        ...
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushLocal(s.type(), offset, argCount, s.isReference(), isArrayDecl, parent.getUserClassRegistry()));
                        code.addInstruction(new StackInstrs.Pop());
                    }
                }
            }
            ...
            case ChuckAST.ReturnStmt s -> {
                ...
                code.addInstruction(parent.getCurrentClass() != null ? new ControlInstrs.ReturnMethod() : new ReturnFunc());
            }
            case ChuckAST.ClassDefStmt s -> {
                ...
                parent.getUserClassRegistry().put(s.name(), new UserClassDescriptor(
