    ...
    void emitExpression(ChuckAST.Exp exp, ChuckCode code) {
        ...
        switch (exp) {
            ...
            case ChuckAST.DeclExp e -> {
                ...
                boolean useGlobal = forceGlobal || (parent.getLocalScopes().size() <= 1 && parent.getCurrentClass() == null);
                ...
                if (isUserClass && !isBuiltinPseudoClass && e.callArgs() instanceof ChuckAST.CallExp call) {
                    ...
                    if (parent.isInPreCtor()) {
                        ...
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushField(parent.getBaseType(e.type()), e.name(), argCount, e.isReference(), false, parent.getUserClassRegistry()));
                    } else {
                        ...
                        Integer localOffset = (forceGlobal || useGlobal || isField) ? null : parent.getLocalOffset(e.name());
                        if (localOffset != null) {
                            code.addInstruction(new ObjectInstrs.InstantiateSetAndPushLocal(e.type(), localOffset, argCount, e.isReference(), false, parent.getUserClassRegistry()));
                            ...
                            code.addInstruction(new ObjectInstrs.InstantiateSetAndPushGlobal(parent.getBaseType(e.type()), e.name(), argCount, e.isReference(), false, parent.getUserClassRegistry()));
                        }
                    }
                    ...
                    if (parent.isInPreCtor()) {
                        code.addInstruction(new StackInstrs.PushThis());
                        code.addInstruction(new ObjectInstrs.InstantiateSetAndPushField(parent.getBaseType(e.type()), e.name(), argCount, e.isReference(), isArrayDecl, parent.getUserClassRegistry()));
                    } else {
                        boolean isField = parent.getCurrentClass() != null && (parent.getCurrentClassFields().contains(e.name()) || parent.hasInstanceField(parent.getCurrentClass(), e.name()));
                        Integer localOffset = (forceGlobal || useGlobal || isField) ? null : parent.getLocalOffset(e.name());
                        if (localOffset != null) {
                            code.addInstruction(new ObjectInstrs.InstantiateSetAndPushLocal(e.type(), localOffset, argCount, e.isReference(), isArrayDecl, parent.getUserClassRegistry()));
                        } else if (!forceGlobal && !useGlobal && !isField && !parent.getLocalScopes().isEmpty()) {
                            Map<String, Integer> scope = parent.getLocalScopes().peek();
                            localOffset = parent.getLocalCount();
                            parent.setLocalCount(localOffset + 1);
                            ...
                            code.addInstruction(new ObjectInstrs.InstantiateSetAndPushGlobal(parent.getBaseType(e.type()), e.name(), argCount, e.isReference(), isArrayDecl, parent.getUserClassRegistry()));
                        }
                    }
                }
            }
            case ChuckAST.BinaryExp e -> {
                if (e.op() == ChuckAST.Operator.CHUCK || e.op() == ChuckAST.Operator.AT_CHUCK) {
                    ...
                    if (e.rhs() instanceof ChuckAST.DeclExp rDecl) {
                        ...
                        boolean resolvedUseGlobal = resolvedDecl.isGlobal() || (parent.getLocalScopes().size() <= 1 && parent.getCurrentClass() == null);
                    ...
                    parent.emitChuckTarget(e.rhs(), code, e.op());
                } else if (e.op() == ChuckAST.Operator.WRITE_IO || (e.op() == ChuckAST.Operator.LE && parent.isIOType(parent.getExprType(e.lhs())))) {
                    this.emitExpression(e.lhs(), code);
                    this.emitExpression(e.rhs(), code);
                        ...
                        if (lhsType != null && parent.getUserClassRegistry().containsKey(lhsType)) {
                            ...
                            if (opSymbol != null) {
                                ...
                                if (opFunc == null) {
                                    ...
                                    if (opFunc == null) opFunc = parent.getFunctions().get("__op__" + opSymbol + ":2");
                                }
                                if (opFunc != null) {
                                    this.emitExpression(e.lhs(), code);
                                    this.emitExpression(e.rhs(), code);
                                ...
                                if (desc != null) {
                                    ...
                                    String mPubKey = parent.resolveMethodKey(lhsType, "__pub_op__" + opSymbol, mArgTypes);
                                    String mPrivKey = parent.resolveMethodKey(lhsType, "__op__" + opSymbol, mArgTypes);
                    ...
                    }
                    {
                        ...
                        if (e.op() == ChuckAST.Operator.TIMES && ("float".equals(lhsTypeS) || "int".equals(lhsTypeS)) && ChuckEmitter.isVecType(rhsTypeS)) {
                            this.emitExpression(e.rhs(), code); this.emitExpression(e.lhs(), code);
                            code.addInstruction(new VecInstrs.VecScale());
                            return;
                        }
                    }
                    {
                        ...
                        if (ChuckEmitter.isVecType(lhsType)) {
                            switch (e.op()) {
                                ...
                                case TIMES -> {
                                    String rhsType = parent.getExprType(e.rhs());
                                    if ("complex".equals(lhsType) && "complex".equals(rhsType)) { this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new ComplexInstrs.Mul()); return; }
                                    if ("polar".equals(lhsType) && "polar".equals(rhsType)) { this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new PolarInstrs.Mul()); return; }
                                    if (rhsType == null || "float".equals(rhsType) || "int".equals(rhsType)) { this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new VecInstrs.VecScale()); return; }
                                    if (("vec3".equals(lhsType) || "vec4".equals(lhsType)) && ("vec3".equals(rhsType) || "vec4".equals(rhsType))) { this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new VecInstrs.Cross()); return; }
                                    this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new VecInstrs.Dot()); return;
                                }
                                case DIVIDE -> {
                                    String rhsType = parent.getExprType(e.rhs());
                                    if ("complex".equals(lhsType) && "complex".equals(rhsType)) { this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new ComplexInstrs.Div()); return; }
                                    if ("polar".equals(lhsType) && "polar".equals(rhsType)) { this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code); code.addInstruction(new PolarInstrs.Div()); return; }
                                    if (rhsType == null || "float".equals(rhsType) || "int".equals(rhsType)) {
                                        this.emitExpression(e.lhs(), code); this.emitExpression(e.rhs(), code);
                                        code.addInstruction(new PushInstrs.PushFloat(1.0)); code.addInstruction(new StackInstrs.Swap());
                                        code.addInstruction(new ArithmeticInstrs.DivideAny()); code.addInstruction(new VecInstrs.VecScale());
                                        return;
                                    }
                                }
                                default -> { }
                    ...
                    switch (e.op()) {
                        ...
                        case LE -> {
                            ...
                            if ("IO".equals(lt) || "FileIO".equals(lt)) code.addInstruction(new MiscInstrs.ChuckWriteIO());
                        ...
                        case DUR_MUL -> {
                            this.emitExpression(e.lhs(), code);
                            if (e.rhs() instanceof ChuckAST.IdExp id) code.addInstruction(new MiscInstrs.CreateDuration(id.name()));
                        ...
                        case AND -> {
                            ...
                            if ("Event".equals(lt) || "Event".equals(rt)) {
                                ...
                                this.emitExpression(e.lhs(), code);
                                int jumpIdx = code.getNumInstructions(); code.addInstruction(null);
                                this.emitExpression(e.rhs(), code);
                        ...
                        case SHIFT_LEFT -> code.addInstruction(new ArrayInstrs.ShiftLeftOrAppend(parent.getExprType(e.lhs())));
                        default -> { }
                    }
                }
            }
            case ChuckAST.IdExp e -> {
                ...
                else if (parent.getCurrentClass() != null && parent.hasStaticField(parent.getCurrentClass(), e.name())) code.addInstruction(new FieldInstrs.GetStatic(parent.getCurrentClass(), e.name()));
                ...
                else if (Set.of("second", "ms", "samp", "minute", "hour").contains(e.name())) { code.addInstruction(new PushInstrs.PushInt(1)); code.addInstruction(new MiscInstrs.CreateDuration(e.name())); }
            ...
            case ChuckAST.DotExp e -> {
                if (e.base() instanceof ChuckAST.IdExp supId && supId.name().equals("super")) { code.addInstruction(new StackInstrs.PushThis()); code.addInstruction(new FieldInstrs.GetFieldByName(e.member())); return; }
                ...
                if (e.base() instanceof ChuckAST.IdExp id && id.name().equals("IO")) {
                    if (e.member().equals("nl") || e.member().equals("newline")) code.addInstruction(new PushInstrs.PushString("\n"));
                    else code.addInstruction(new FieldInstrs.GetBuiltinStatic("org.chuck.core.ChuckIO", e.member()));
                    return;
                }
                if (e.base() instanceof ChuckAST.IdExp id && id.name().equals("Math")) {
                    switch (e.member()) {
                        case "INFINITY", "infinity" -> code.addInstruction(new PushInstrs.PushFloat(Double.POSITIVE_INFINITY));
                        case "NEGATIVE_INFINITY" -> code.addInstruction(new PushInstrs.PushFloat(Double.NEGATIVE_INFINITY));
                        ...
                        case "HALF_PI", "half_pi" -> code.addInstruction(new PushInstrs.PushFloat(Math.PI / 2.0));
                        ...
                        case "j" -> { code.addInstruction(new PushInstrs.PushFloat(0.0)); code.addInstruction(new PushInstrs.PushFloat(1.0)); code.addInstruction(new ArrayInstrs.NewArrayFromStack(2, "complex")); }
                    }
                    return;
                }
                if (e.member().equals("size")) { this.emitExpression(e.base(), code); code.addInstruction(new ObjectInstrs.CallMethod("size", 0)); return; }
                else if (e.member().equals("zero")) { this.emitExpression(e.base(), code); code.addInstruction(new ArrayInstrs.ArrayZero()); return; }
                if (e.base() instanceof ChuckAST.IdExp id) {
                    String bt = parent.getExprType(e.base());
                    if (bt != null && parent.getUserClassRegistry().containsKey(bt) && (parent.getUserClassRegistry().get(bt).staticInts().containsKey(e.member()) || parent.getUserClassRegistry().get(bt).staticObjects().containsKey(e.member()))) { code.addInstruction(new FieldInstrs.GetStatic(bt, e.member())); return; }
                    if (Set.of("ADSR", "Adsr").contains(id.name())) { code.addInstruction(new FieldInstrs.GetBuiltinStatic("org.chuck.audio.Adsr", e.member())); return; }
                    if (id.name().equals("Std")) { code.addInstruction(new FieldInstrs.GetBuiltinStatic("org.chuck.core.Std", e.member())); return; }
                    if (id.name().equals("Machine")) {
                        switch (e.member()) {
                            ...
                            case "version", "platform", "os", "loglevel", "timeofday" -> code.addInstruction(new org.chuck.core.instr.MachineCall(e.member(), 0));
                            default -> code.addInstruction(new org.chuck.core.instr.MachineCall(e.member(), 0));
                        }
                        return;
                    }
                    if (Set.of("RegEx", "Reflect", "SerialIO", "FileIO").contains(id.name())) { code.addInstruction(new FieldInstrs.GetBuiltinStatic("org.chuck.core." + id.name(), e.member())); return; }
                    if (parent.getUserClassRegistry().containsKey(id.name())) {
                        UserClassDescriptor d = parent.getUserClassRegistry().get(id.name());
                        if (d.staticInts().containsKey(e.member()) || d.staticObjects().containsKey(e.member())) { code.addInstruction(new FieldInstrs.GetStatic(id.name(), e.member())); return; }
                    }
                    if (parent.getCurrentClass() != null && parent.getUserClassRegistry().containsKey(parent.getCurrentClass()) && parent.getUserClassRegistry().get(parent.getCurrentClass()).staticObjects().containsKey(id.name())) {
                        code.addInstruction(new FieldInstrs.GetStatic(parent.getCurrentClass(), id.name())); code.addInstruction(new FieldInstrs.GetFieldByName(e.member())); return;
                    }
                }
                if (ChuckEmitter.vecFieldIndex(e.member()) >= 0 && ChuckEmitter.isVecType(parent.getExprType(e.base()))) { this.emitExpression(e.base(), code); code.addInstruction(new FieldInstrs.GetFieldByName(e.member())); return; }
                this.emitExpression(e.base(), code);
                String baseType = parent.getExprType(e.base());
                if (baseType != null) {
                    if (parent.getUserClassRegistry().containsKey(baseType)) parent.checkAccess(baseType, e.member(), false, e.line(), e.column());
                    if (parent.isKnownUGenType(baseType) || parent.isSubclassOfUGen(baseType)) code.addInstruction(new ObjectInstrs.CallMethod(e.member(), 0));
                    else code.addInstruction(new FieldInstrs.GetFieldByName(e.member()));
                } else code.addInstruction(new FieldInstrs.GetFieldByName(e.member()));
            }
            case ChuckAST.ArrayLitExp e -> { for (ChuckAST.Exp el : e.elements()) this.emitExpression(el, code); code.addInstruction(new ArrayInstrs.NewArrayFromStack(e.elements().size())); }
            case ChuckAST.VectorLitExp e -> {
                for (ChuckAST.Exp el : e.elements()) { this.emitExpression(el, code); code.addInstruction(new TypeInstrs.EnsureFloat()); }
                String vTag = switch (e.elements().size()) { case 2 -> "vec2"; case 3 -> "vec3"; case 4 -> "vec4"; default -> null; };
                code.addInstruction(new ArrayInstrs.NewArrayFromStack(e.elements().size(), vTag));
            }
            case ChuckAST.ComplexLit e -> { this.emitExpression(e.re(), code); this.emitExpression(e.im(), code); code.addInstruction(new ArrayInstrs.NewArrayFromStack(2, "complex")); }
            case ChuckAST.PolarLit e -> { this.emitExpression(e.mag(), code); this.emitExpression(e.phase(), code); code.addInstruction(new ArrayInstrs.NewArrayFromStack(2, "polar")); }
            case ChuckAST.ArrayAccessExp e -> { this.emitExpression(e.base(), code); for (ChuckAST.Exp index : e.indices()) { this.emitExpression(index, code); code.addInstruction(new ArrayInstrs.GetArrayInt()); } }
            case ChuckAST.CallExp e -> {
                ...
                if (e.base() instanceof ChuckAST.DotExp dot && dot.base() instanceof ChuckAST.IdExp baseId) {
                    ...
                    if (bn.equals("IO") && Set.of("nl", "newline").contains(mn)) { code.addInstruction(new PushInstrs.PushString("\n")); return; }
                    if (bn.equals("Std") && Set.of("mtof", "ftom", "powtodb", "rmstodb", "dbtopow", "dbtorms", "dbtolin", "lintodb", "abs", "fabs", "sgn", "rand2", "rand2f", "clamp", "clampf", "scalef", "atoi", "atof", "itoa", "ftoi", "systemTime", "range", "getenv", "setenv").contains(mn)) {
                        for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new MathInstrs.StdFunc(mn, argc)); return;
                    }
                    if (bn.equals("Math")) {
                        if (Set.of("random", "randf").contains(mn)) { code.addInstruction(new MathInstrs.MathRandom()); return; }
                        if (mn.equals("help")) { code.addInstruction(new MathInstrs.MathHelp()); return; }
                        for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new MathInstrs.MathFunc(mn)); return;
                    }
                    if (bn.equals("Machine")) {
                        if (mn.equals("realtime")) { code.addInstruction(new PushInstrs.PushInt(0)); return; }
                        if (mn.equals("silent")) { code.addInstruction(new PushInstrs.PushInt(1)); return; }
                        for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new org.chuck.core.instr.MachineCall(mn, argc)); return;
                    }
                    if (Set.of("RegEx", "Reflect", "SerialIO").contains(bn)) { for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.CallBuiltinStatic("org.chuck.core." + bn, mn, argc)); return; }
                }
                if (e.base() instanceof ChuckAST.DotExp dot && (dot.base() instanceof ChuckAST.MeExp || (dot.base() instanceof ChuckAST.IdExp idMe && idMe.name().equals("me")))) {
                    ...
                    switch (mn) {
                        ...
                        case "dir", "arg" -> { if (!e.args().isEmpty()) this.emitExpression(e.args().get(0), code); else code.addInstruction(new PushInstrs.PushInt(0)); if (mn.equals("dir")) code.addInstruction(new MeInstrs.MeDir()); else code.addInstruction(new MeInstrs.MeArg()); return; }
                    ...
                    code.addInstruction(new PushInstrs.PushMe()); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code);
                    String fk = parent.resolveMethodKey("Shred", mn, argTypes); code.addInstruction(new ObjectInstrs.CallMethod(mn, argc, fk)); return;
                }
                if (e.base() instanceof ChuckAST.DotExp dot && dot.base() instanceof ChuckAST.IdExp supId && supId.name().equals("super") && parent.getCurrentClass() != null) {
                    UserClassDescriptor cd = parent.getUserClassRegistry().get(parent.getCurrentClass());
                    if (cd != null && cd.parentName() != null) { for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.CallSuperMethod(cd.parentName(), dot.member(), argc)); return; }
                }
                if (e.base() instanceof ChuckAST.DotExp dot) {
                    String bt = parent.getExprType(dot.base());
                    if (bt == null && dot.base() instanceof ChuckAST.IdExp id && parent.getUserClassRegistry().containsKey(id.name())) bt = id.name();
                    if (bt != null) {
                        if (bt.equals("string") || bt.endsWith("[]")) { this.emitExpression(dot.base(), code); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); String fk = parent.getMethodKey(dot.member(), argTypes); code.addInstruction(new ObjectInstrs.CallMethod(dot.member(), argc, fk)); return; }
                        if (dot.member().equals("last") && (parent.isKnownUGenType(bt) || parent.isSubclassOfUGen(bt))) { this.emitExpression(dot.base(), code); code.addInstruction(new UgenInstrs.GetLastOut()); return; }
                        if (dot.member().equals("dot") && ChuckEmitter.isVecType(bt)) { this.emitExpression(dot.base(), code); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new VecInstrs.Dot()); return; }
                        if (parent.getUserClassRegistry().containsKey(bt)) {
                            String fk = parent.getMethodKey(dot.member(), argTypes); ChuckCode tgt = parent.resolveStaticMethod(bt, fk);
                            if (tgt == null) tgt = parent.resolveStaticMethod(bt, dot.member() + ":" + argc);
                            if (tgt != null) { String finalK = (tgt.getName().equals(dot.member())) ? fk : dot.member() + ":" + argc; parent.checkAccess(bt, finalK, true, e.line(), e.column()); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new CallFunc(tgt, argc)); return; }
                        }
                        this.emitExpression(dot.base(), code); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code);
                        String fk = parent.resolveMethodKey(bt, dot.member(), argTypes); if (parent.getUserClassRegistry().containsKey(bt)) parent.checkAccess(bt, fk, true, e.line(), e.column());
                        code.addInstruction(new ObjectInstrs.CallMethod(dot.member(), argc, fk)); return;
                    }
                } else if (e.base() instanceof ChuckAST.IdExp id) {
                    String name = id.name();
                    if (parent.getCurrentClass() != null && (name.equals(parent.getCurrentClass()) || name.equals("this"))) {
                        String ck = parent.getMethodKey(parent.getCurrentClass(), argTypes); code.addInstruction(new StackInstrs.PushThis()); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.CallMethod(parent.getCurrentClass(), argc, ck)); return;
                    }
                    if (parent.getCurrentClass() != null) {
                        String fk = parent.getMethodKey(name, argTypes); ChuckCode tgt = parent.resolveStaticMethod(parent.getCurrentClass(), fk);
                        if (tgt == null) tgt = parent.resolveStaticMethod(parent.getCurrentClass(), name + ":" + argc);
                        if (tgt != null) { for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new CallFunc(tgt, argc)); return; }
                        UserClassDescriptor desc = parent.getUserClassRegistry().get(parent.getCurrentClass());
                        if (desc != null && desc.methods().containsKey(fk)) { code.addInstruction(new StackInstrs.PushThis()); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.CallMethod(name, argc, fk)); return; }
                    }
                    String fk = parent.getMethodKey(name, argTypes); if (parent.getFunctions().containsKey(fk)) { for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new CallFunc(parent.getFunctions().get(fk), argc)); return; }
                    String fbk = name + ":" + argc; if (parent.getFunctions().containsKey(fbk)) { for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new CallFunc(parent.getFunctions().get(fbk), argc)); return; }
                    if (Set.of("print", "chout", "cherr").contains(name)) { for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new ChuckPrint(argc)); return; }
                }
                this.emitExpression(e.base(), code); for (ChuckAST.Exp arg : e.args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.CallMethod("unknown", argc));
            }
            case ChuckAST.SporkExp e -> {
                String fn = null;
                switch (e.call().base()) {
                    case ChuckAST.IdExp id -> {
                        fn = id.name(); List<String> ats = new ArrayList<>(); for (ChuckAST.Exp arg : e.call().args()) ats.add(parent.getExprType(arg));
                        String key = parent.getMethodKey(fn, ats); ChuckCode tgt = parent.resolveStaticMethod(parent.getCurrentClass(), key);
                        if (tgt == null) tgt = parent.resolveStaticMethod(parent.getCurrentClass(), fn + ":" + e.call().args().size());
                        if (tgt != null) { for (ChuckAST.Exp arg : e.call().args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.Spork(tgt, e.call().args().size())); return; }
                        if (parent.getFunctions().containsKey(key)) { for (ChuckAST.Exp arg : e.call().args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.Spork(parent.getFunctions().get(key), e.call().args().size())); return; }
                        String fbk = fn + ":" + e.call().args().size(); if (parent.getFunctions().containsKey(fbk)) { for (ChuckAST.Exp arg : e.call().args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.Spork(parent.getFunctions().get(fbk), e.call().args().size())); return; }
                    }
                    case ChuckAST.DotExp dot -> {
                        List<String> ats = e.call().args().stream().map(parent::getExprType).toList(); String bcn = null;
                        if (dot.base() instanceof ChuckAST.IdExp id && parent.getUserClassRegistry().containsKey(id.name())) bcn = id.name();
                        else { String bt = parent.getExprType(dot.base()); if (bt != null && parent.getUserClassRegistry().containsKey(bt)) bcn = bt; }
                        if (bcn != null) { String rk = parent.resolveMethodKey(bcn, dot.member(), ats); ChuckCode tgt = parent.resolveStaticMethod(bcn, rk); if (tgt != null) { for (ChuckAST.Exp arg : e.call().args()) this.emitExpression(arg, code); code.addInstruction(new ObjectInstrs.Spork(tgt, e.call().args().size())); return; } }
                        this.emitExpression(dot.base(), code); for (ChuckAST.Exp arg : e.call().args()) this.emitExpression(arg, code);
                        String bt = parent.getExprType(dot.base()); String rk = (bt != null) ? parent.resolveMethodKey(bt, dot.member(), ats) : parent.getMethodKey(dot.member(), ats);
                        code.addInstruction(new ObjectInstrs.SporkMethod(dot.member(), e.call().args().size(), rk)); return;
                    }
                    default -> { }
                }
                this.emitExpression(e.call(), code);
            }
            case ChuckAST.TernaryExp e -> { this.emitExpression(e.condition(), code); int jf = code.getNumInstructions(); code.addInstruction(null); this.emitExpression(e.thenExp(), code); int je = code.getNumInstructions(); code.addInstruction(null); code.replaceInstruction(jf, new ControlInstrs.JumpIfFalse(code.getNumInstructions())); this.emitExpression(e.elseExp(), code); code.replaceInstruction(je, new ControlInstrs.Jump(code.getNumInstructions())); }
            case ChuckAST.CastExp e -> { this.emitExpression(e.value(), code); switch (e.targetType()) { case "int" -> code.addInstruction(new TypeInstrs.CastToInt()); case "float" -> code.addInstruction(new TypeInstrs.CastToFloat()); case "string" -> code.addInstruction(new TypeInstrs.CastToString()); case "complex" -> code.addInstruction(new TypeInstrs.CastToComplex()); case "polar" -> code.addInstruction(new TypeInstrs.CastToPolar()); } }
