...
/**
 * Sets a named member on a ChucK object using reflection. Stack layout at execution: [value ...
 * object] (object on top).
 */
public class SetMemberIntByName implements ChuckInstr {
  ...
  public static final Map<String, Integer> MEMBER_OFFSETS =
      Map.of(
          "freq", 0,
          "gain", 1,
          "width", 2,
  ...
  public void execute(ChuckVM vm, ChuckShred shred) {
    ...
    String setter = "set" + Character.toUpperCase(memberName.charAt(0)) + memberName.substring(1);
    ...
    if (isObjVal) {
      ...
      if (rawObj instanceof ChuckArray arr && arr.vecTag != null) {
        if (arr.vecTag.equals("complex")) {
          if (memberName.equals("re")) {
            arr.setFloat(0, doubleVal);
            called = true;
          } else if (memberName.equals("im")) {
            arr.setFloat(1, doubleVal);
            called = true;
          }
        } else if (arr.vecTag.equals("polar")) {
          if (memberName.equals("mag")) {
            arr.setFloat(0, doubleVal);
            called = true;
          } else if (memberName.equals("phase")) {
            arr.setFloat(1, doubleVal);
            called = true;
          }
