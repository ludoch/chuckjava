  ...
  public static class MathFunc implements ChuckInstr {
    ...
    public void execute(ChuckVM vm, ChuckShred s) {
        ...
        if (obj instanceof ChuckArray a) {
          if ("complex".equals(a.vecTag) || "polar".equals(a.vecTag)) {
            ...
            switch (fn) {
              ...
              case "mag" -> {
                s.reg.push(Math.sqrt(re * re + im * im));
                return;
              }
              case "phase" -> {
                s.reg.push(Math.atan2(im, re));
                return;
              }
              case "abs" -> {
                s.reg.push(Math.sqrt(re * re + im * im));
              ...
              case "isnan" -> {
                s.reg.push((Double.isNaN(re) || Double.isNaN(im)) ? 1L : 0L);
                return;
              }
              case "isinf" -> {
                s.reg.push((Double.isInfinite(re) || Double.isInfinite(im)) ? 1L : 0L);
                return;
              }
            }
            ...
            switch (fn) {
              case "sin" -> {
                res.setFloat(0, Math.sin(re) * Math.cosh(im));
                res.setFloat(1, Math.cos(re) * Math.sinh(im));
              }
              case "cos" -> {
                res.setFloat(0, Math.cos(re) * Math.cosh(im));
                res.setFloat(1, -Math.sin(re) * Math.sinh(im));
              }
              case "tan" -> {
              ...
              case "asin" -> {
                ...
                double r = Math.sqrt(Math.sqrt(dre * dre + dim * dim)),
                    t = Math.atan2(dim, dre) / 2.0;
              ...
              case "atan" -> {
                ...
                res.setFloat(0, -qt / 2.0);
                res.setFloat(1, Math.log(qr) / 2.0);
              }
              case "atan2" -> {
                double bre, bim;
                if (s.reg.isObject(0)) {
                  ...
                  if (bObj instanceof ChuckArray b) {
                    if ("polar".equals(b.vecTag)) {
                      ...
                      bre = bmag * Math.cos(bph);
                      bim = bmag * Math.sin(bph);
                    } else {
                      bre = b.getFloat(0);
                      bim = b.getFloat(1);
                    }
                  ...
                  bre = s.reg.popAsDouble();
                  bim = 0;
                }
                res.setFloat(0, Math.atan2(re, bre));
                res.setFloat(1, 0.0);
              }
              case "pow" -> {
                double bre, bim;
                ...
                res.setFloat(0, expAre * Math.cos(aim));
                res.setFloat(1, expAre * Math.sin(aim));
              }
              case "sqrt" -> {
                double r = Math.sqrt(Math.sqrt(re * re + im * im)),
                    theta = Math.atan2(im, re) / 2.0;
                res.setFloat(0, r * Math.cos(theta));
                res.setFloat(1, r * Math.sin(theta));
              }
              case "floor" -> {
                res.setFloat(0, Math.floor(re));
                res.setFloat(1, Math.floor(im));
              }
              case "ceil" -> {
                res.setFloat(0, Math.ceil(re));
                res.setFloat(1, Math.ceil(im));
              }
              case "log" -> {
                res.setFloat(0, Math.log(Math.sqrt(re * re + im * im)));
                res.setFloat(1, Math.atan2(im, re));
              }
              case "exp" -> {
                double expRe = Math.exp(re);
                res.setFloat(0, expRe * Math.cos(im));
                res.setFloat(1, expRe * Math.sin(im));
              }
              default -> {
                res.setFloat(0, 0.0);
                res.setFloat(1, 0.0);
      ...
      switch (fn) {
        ...
        case "pow" -> {
          double exp = s.reg.popAsDouble(), base = s.reg.popAsDouble();
          s.reg.push(Math.pow(base, exp));
        }
        case "atan2" -> {
          double x = s.reg.popAsDouble(), y = s.reg.popAsDouble();
          s.reg.push(Math.atan2(y, x));
        }
        case "srandom" -> {
        ...
        // ── Two-arg functions ────────────────────────────────────────
        case "hypot" -> {
          double y = s.reg.popAsDouble(), x = s.reg.popAsDouble();
          s.reg.push(Math.hypot(x, y));
        }
        case "fmod" -> {
          double y = s.reg.popAsDouble(), x = s.reg.popAsDouble();
          s.reg.push(x % y);
        ...
        case "remainder" -> {
          double y = s.reg.popAsDouble(), x = s.reg.popAsDouble();
          s.reg.push(Math.IEEEremainder(x, y));
        }
        case "min" -> {
          double b = s.reg.popAsDouble(), a = s.reg.popAsDouble();
          s.reg.push(Math.min(a, b));
        }
        case "max" -> {
          double b = s.reg.popAsDouble(), a = s.reg.popAsDouble();
          s.reg.push(Math.max(a, b));
        }
        case "random2", "random2i" -> {
          long hi = s.reg.popLong(), lo = s.reg.popLong();
          s.reg.push(Std.rand2(lo, hi));
        }
        case "random2f" -> {
          double hi = s.reg.popAsDouble(), lo = s.reg.popAsDouble();
          s.reg.push(Std.rand2f(lo, hi));
        }
        case "randomf" -> s.reg.push(Std.rng.nextDouble());
        case "gauss" -> {
          double std = s.reg.popAsDouble(), mean = s.reg.popAsDouble();
          s.reg.push(mean + std * Std.rng.nextGaussian());
        }
        // ── Multi-arg functions ───────────────────────────────────────
        case "map", "scalef" -> {
        ...
        case "clampi" -> {
          long hi = s.reg.popLong(), lo = s.reg.popLong(), x = s.reg.popLong();
          s.reg.push(Std.clamp(x, lo, hi));
        }
        case "clampf" -> {
          double hi = s.reg.popAsDouble(), lo = s.reg.popAsDouble(), x = s.reg.popAsDouble();
          s.reg.push(Std.clampf(x, lo, hi));
        }
        // ── Integer result ────────────────────────────────────────────
