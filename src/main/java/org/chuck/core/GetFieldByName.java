...
/**
 * Pops an object from the reg stack and pushes the named field's value. Used for 'f.x' read access
 * from outside the class (top-level code). Falls back gracefully for non-UserObject targets.
 */
public class GetFieldByName implements ChuckInstr {
  private final String name;

  public GetFieldByName(String name) {
    this.name = name;
  }
