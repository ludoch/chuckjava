 ...
 * train(data[][]) — computes principal components
 * transform(input[], output[]) — projects input onto reduced space
...
public class PCA extends ChuckObject {
    ...
    public long numComponents(long n) { numComponents = (int) n; return n; }
    ...
    public long train(ChuckArray dataArr) {
        ...
        for (double[] row : X) for (int i = 0; i < D; i++) for (int j = 0; j < D; j++) cov[i][j] += row[i] * row[j];
        ...
        double totalVar = 0; for (int i = 0; i < D; i++) totalVar += cov[i][i];
    ...
    public long transform(ChuckArray inArr, ChuckArray outArr) {
        ...
        for (int pc = 0; pc < k; pc++) {
            double dot = 0; for (int d = 0; d < D; d++) dot += components[pc][d] * centered[d];
    ...
    private static double[] powerIter(double[][] mat, int D) {
        double[] v = new double[D]; v[0] = 1;
        for (int iter = 0; iter < 200; iter++) {
            double[] next = new double[D];
            for (int i = 0; i < D; i++) for (int j = 0; j < D; j++) next[i] += mat[i][j] * v[j];
            double norm = 0; for (double x : next) norm += x * x; norm = Math.sqrt(norm);
            if (norm < 1e-12) break;
    ...
    private static double rayleigh(double[][] mat, double[] v, int D) {
        double num = 0;
        for (int i = 0; i < D; i++) { double mv = 0; for (int j = 0; j < D; j++) mv += mat[i][j] * v[j]; num += v[i] * mv; }
        return num;
    }
}
