...
/**
 * Multi-Layer Perceptron — fully-connected feedforward network. Architecture: configurable
 * input/hidden/output dimensions. Training: mini-batch SGD with backpropagation (sigmoid
 * activation, MSE loss).
 */
        ...
        for (int l = 0; l < L; l++) {
          ...
          for (int j = 0; j < weights[l].length; j++) {
            for (int i = 0; i < in.length; i++)
              weights[l][j][i] -= learningRate * deltas[l][j] * in[i];
