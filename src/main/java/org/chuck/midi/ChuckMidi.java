 ...
 *
 * <p>Usage from ChucK code: MidiIn min; MidiMsg msg; min.open(0); // open port 0 while (true) { min
 * => now; // wait for next message min.recv(msg); // copy bytes into msg msg.data1 => ...; }
 */
public class ChuckMidi {
  ...
  /**
   * Opens the MIDI input device at the given port index (0-based among input devices). Returns true
   * on success.
   */
  ...
  /**
   * Copies the next queued message into msg. Returns true if a message was available. Intended to
   * be called right after a shred wakes from "min => now".
   */
