...
/**
 * ChucK MidiFileIn — reads Standard MIDI (.mid) files for sequencing.
 * Usage in ChucK:
 *   MidiFileIn mf;
 *   MidiMsg msg;
 *   mf.open("song.mid");
...
public class MidiFileIn extends ChuckObject {
    ...
    /**
     * Open a MIDI file. Returns 1 on success, 0 on failure.
     * All tracks are merged into a single chronologically sorted message list.
     */
    ...
    public long more() { return (opened && cursor < messages.size()) ? 1L : 0L; }

    /** Total number of MIDI messages across all tracks. */
    public long size() { return messages.size(); }
