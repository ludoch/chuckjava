...
/**
 * Holds one MIDI message as three raw bytes, matching ChucK's MidiMsg convention:
 *   data1 = status byte  (e.g. 0x90 = note-on ch 1)
 *   data2 = first data   (note number 0-127, or controller number)
...
public class MidiMsg extends ChuckObject {
    ...
    public boolean isNoteOn()  { return (data1 & 0xF0) == 0x90 && data3 > 0; }
    /** Convenience: true when this is a Note-Off or Note-On with velocity 0. */
    public boolean isNoteOff() { return (data1 & 0xF0) == 0x80 || ((data1 & 0xF0) == 0x90 && data3 == 0); }
    /** MIDI channel, 0-indexed. */
    public int channel()       { return data1 & 0x0F; }
