...
/**
 * Chroma — 12-bin pitch-class profile (chromagram) UAna.
 * Chains after FFT: maps FFT magnitude bins to the 12 semitone pitch classes.
 * Useful for key/chord detection.
 *
 * Output: 12 fvals corresponding to [C, C#, D, D#, E, F, F#, G, G#, A, A#, B].
 *
 * Usage in ChucK:
 *   adc => FFT fft => Chroma chroma => blackhole;
 *   fft.size(2048);
 *   while (true) { 2048::samp => now; chroma.upchuck() @=> UAnaBlob blob; }
 */
public class Chroma extends UAna {
    private float sampleRate;

    public Chroma() { this(44100f); }
    public Chroma(float sr) { this.sampleRate = sr; }

    public void setSampleRate(float sr) { this.sampleRate = sr; }
