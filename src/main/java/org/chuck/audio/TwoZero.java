...
/**
 * Two-zero (FIR) notch filter.
 * H(z) = b0 + b1*z^-1 + b2*z^-2
 *
 * Use setNotch(freq, radius) for a normalized notch, or set raw b0/b1/b2.
 */
public class TwoZero extends ChuckUGen {
    ...
    /**
     * Place two zeros at the given frequency (Hz) and radius, normalized so
     * gain = 1 everywhere except the notch.
     */
    public void setNotch(double frequency, double radius) {
        ...
        double norm;
        if (b1 > 0.0)
            norm = 1.0 / (1.0 + b1 + b2);  // peak at z = -1
        else
            norm = 1.0 / (1.0 - b1 + b2);  // peak at z = +1
        b0 = norm;
        b1 *= norm;
    ...
    public double freq(double f)   { notchFreq = f; setNotch(notchFreq, notchRad); return f; }
    public double freq()           { return notchFreq; }
    public double radius(double r) { notchRad  = r; setNotch(notchFreq, notchRad); return r; }
    public double radius()         { return notchRad; }
