...
/**
 * FrencHrn — FM French horn.
 * Uses two-operator FM with a ratio of ~1:1 and slow attack,
 * with an additional even harmonic for the horn's warmth.
 */
public class FrencHrn extends ChuckUGen {
    ...
    public FrencHrn(float sr) {
        this.sampleRate = sr;
        mod = new SinOsc(sr); car = new SinOsc(sr); sub = new SinOsc(sr);
        env = new Adsr(sr);
