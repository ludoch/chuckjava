 ...
 *
 * Measures the "tailedness" or impulsiveness of the spectral distribution.
 *   High kurtosis → impulsive/transient sounds.
 *   Low/negative kurtosis → sustained/tonal sounds.
 *
...
public class Kurtosis extends UAna {
    ...
    public void computeFromSpectrum(float[] mags) {
        if (mags == null || mags.length < 2) { result = 0.0f; return; }
        double sum = 0.0;
        for (float m : mags) sum += m;
