...
/**
 * AutoCorr — Autocorrelation UAna. Accumulates audio in a buffer; on upchuck() computes normalized
 * autocorrelation. Useful for pitch detection and periodicity analysis.
 *
 * <p>Result: R[k] = (1/N) * sum(n=0..N-k-1) x[n] * x[n+k], k = 0..N-1 R[0] = 1.0 when normalized
 * (divide all by R[0]).
 *
 * <p>Usage in ChucK: adc => AutoCorr ac => blackhole; while (true) { 1024::samp => now;
 * ac.upchuck() @=> UAnaBlob blob; }
 */
