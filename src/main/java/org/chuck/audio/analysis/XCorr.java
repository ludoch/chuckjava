...
/**
 * XCorr — Cross-correlation UAna. Cross-correlates the audio inputs from two sources. Connect two
 * signals: sig1 => xcorr; sig2 => xcorr; Result: R[k] = (1/N) * sum(n) x[n] * y[n+k]
 *
 * <p>Usage in ChucK: adc => XCorr xc => blackhole; mic => xc; while (true) { 1024::samp => now;
 * xc.upchuck() @=> UAnaBlob blob; }
 */
