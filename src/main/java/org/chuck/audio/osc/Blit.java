 ...
 *
 * <p>Implements the Stilson-Smith algorithm: output = sin(M * phase) / (M * sin(phase)) where M =
 * 2*nHarmonics+1 is chosen so harmonics stay below Nyquist.
 *
 * <p>This generates a train of band-limited impulses at the set frequency. By default nHarmonics =
 * 0, meaning all harmonics up to sr/2 are included.
 */
public class Blit extends ChuckUGen {
  ...
  private void updateHarmonics() {
    if (nHarmonics <= 0) m = 2 * (int) Math.floor(0.5 * period) + 1;
    else m = 2 * nHarmonics + 1;
  }
