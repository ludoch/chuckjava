 ...
 *
 * <p>Modes (set via mode("name")): "white" — uniform white noise (default) "pink" — pink noise via
 * Voss-McCartney multi-rate algorithm (-3 dB/oct) "brown" — brown/red noise (integrated white, -6
 * dB/oct) "flip" — bit-flip noise (single random bit flipped per sample) "xor" — XOR noise (random
 * bits XOR'd with accumulator)
 *
 * <p>All modes output in approximately [-1, 1].
 */
public class CNoise extends ChuckUGen {
  ...
  private double xor() {
    ...
    long mask = 0;
    for (int i = 0; i < 32; i++) {
      if (rng.nextDouble() < fprob) mask |= (1L << i);
    }
    xorState ^= mask;
    return (xorState & 0xFFFFFFFFL) / (double) 0xFFFFFFFFL * 2.0 - 1.0;
  }
