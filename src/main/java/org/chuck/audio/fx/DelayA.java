 ...
 *
 * <p>Provides fractional delay with flat phase response (unlike DelayL's linear interpolation which
 * has non-flat phase). Uses a first-order allpass filter: y[n] = -c * y[n-1] + x[n-D] + c *
 * x[n-D-1] where c = (1 - alpha) / (1 + alpha) and alpha is the fractional delay part constrained
 * to [0.5, 1.5] for optimal phase behaviour.
 */
public class DelayA extends ChuckUGen {
  ...
  public void setDelaySec(double sec) {
    setDelay(sec * sampleRate);
  }
