 ...
 *
 * Accumulates samples in a ring buffer. Call upchuck() (ChucK: FFT => UAnaBlob)
 * to trigger analysis. Returns size/2 complex bins; fvals() gives magnitudes.
 *
 * Window functions: NONE (0), HANN (1), HAMMING (2), BLACKMAN (3).
 */
public class FFT extends UAna {
    ...
    public int getWindow() { return windowType; }

    private void buildWindow() {
        win = new double[size];
        for (int i = 0; i < size; i++) {
            win[i] = switch (windowType) {
                case WIN_HANN     -> 0.5 * (1.0 - Math.cos(2.0 * Math.PI * i / (size - 1)));
                case WIN_HAMMING  -> 0.54 - 0.46 * Math.cos(2.0 * Math.PI * i / (size - 1));
                case WIN_BLACKMAN -> 0.42 - 0.5  * Math.cos(2.0 * Math.PI * i / (size - 1))
    ...
    /**
     * Run in-place Cooley-Tukey FFT on the current ring buffer contents.
     * Writes size/2 complex bins into lastBlob.
     */
    @Override
        ...
        for (int len = 2; len <= size; len <<= 1) {
            ...
            for (int i = 0; i < size; i += len) {
                ...
                for (int j = 0; j < len / 2; j++) {
                    ...
                    re[u] = uRe + vRe;  im[u] = uIm + vIm;
                    re[v] = uRe - vRe;  im[v] = uIm - vIm;
