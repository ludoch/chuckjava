 ...
 *
 * Use setResonance(freq, radius, normalize) for convenient resonant peak setup,
 * or set raw coefficients via setB0/setA1/setA2.
 */
public class TwoPole extends ChuckUGen {
    ...
    public void setResonance(double frequency, double radius, boolean normalize) {
        ...
        if (normalize) {
            ...
            double real = 1.0 - radius + (a2 - radius)
                    * Math.cos(2.0 * Math.PI * 2.0 * frequency / sampleRate);
            double imag = (a2 - radius)
                    * Math.sin(2.0 * Math.PI * 2.0 * frequency / sampleRate);
    ...
    public double freq(double f)   { resFreq = f; setResonance(resFreq, resRad, resNorm); return f; }
    public double freq()           { return resFreq; }
    public double radius(double r) { resRad  = r; setResonance(resFreq, resRad, resNorm); return r; }
    public double radius()         { return resRad; }
    public double norm(double v)   { setResonance(resFreq, resRad, v != 0.0); return v; }
