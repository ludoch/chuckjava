...
public class HidNative {
  ...
  private static final FunctionDescriptor getRawInputDeviceListDesc =
      FunctionDescriptor.of(
          ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS, ValueLayout.JAVA_INT);
  private static final MethodHandle getRawInputDeviceList =
      user32
          .find("GetRawInputDeviceList")
          .map(addr -> linker.downcallHandle(addr, getRawInputDeviceListDesc))
          .orElse(null);

  // --- WinMM: Joystick Polling ---
  // JOYINFOEX structure size is 52 bytes
  private static final StructLayout JOYINFOEX =
  ...
  private static final FunctionDescriptor joyGetPosExDesc =
      FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.ADDRESS);
  private static final MethodHandle joyGetPosEx =
      winmm
          .find("joyGetPosEx")
          .map(addr -> linker.downcallHandle(addr, joyGetPosExDesc))
          .orElse(null);
