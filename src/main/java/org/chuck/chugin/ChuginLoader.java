...
public class ChuginLoader {
  private static final Logger logger = Logger.getLogger(ChuginLoader.class.getName());
  ...
  public static void loadChugins(String directory) {
    ...
    for (File file : files) {
      try {
        ...
        logger.log(
            Level.SEVERE, "Failed to load chugin: " + file.getName() + " - " + e.getMessage());
      }
    }
  }
      ...
      while (entries.hasMoreElements()) {
        ...
        if (entry.getName().endsWith(".class") && !entry.isDirectory()) {
          ...
          try {
            ...
            if (ChuckObject.class.isAssignableFrom(cls)
                && !cls.isInterface()
                && (cls.getModifiers() & java.lang.reflect.Modifier.ABSTRACT) == 0) {
