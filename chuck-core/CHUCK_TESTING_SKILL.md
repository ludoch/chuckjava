...
```bash
mvn install -pl chuck-core -DskipTests && \
java --enable-preview --add-modules jdk.incubator.vector \
-cp "chuck-core/target/chuck-core-1.0-SNAPSHOT.jar:$(mvn -pl chuck-core dependency:build-classpath | grep -v '\[INFO\]' | tr '\n' ' ')" \
...
-cp "chuck-core/target/chuck-core-1.0-SNAPSHOT.jar:$(mvn -pl chuck-core dependency:build-classpath | grep -v '\[INFO\]' | tr '\n' ' ')" \
org.chuck.SingleTestRunner chuck-core/src/test/01-Basic/116.ck
```
