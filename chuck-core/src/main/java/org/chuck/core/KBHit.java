...
/**
 * ChucK KBHit — non-blocking keyboard press detection. A background virtual thread reads System.in
 * and queues keypresses. Usage in ChucK: KBHit kb; while (true) { kb => now; // wait for a key
 * while (kb.kbhit()) { kb.getchar() => int ch; <<< "key:", ch >>>; } }
 */
public class KBHit extends ChuckObject {
  ...
  /**
   * Return and consume the next pending keypress as its ASCII/Unicode code point, or -1 if no key
   * is waiting.
   */
  public long getchar() {
