...
/**
 * Support Vector Machine — binary and multiclass (one-vs-rest). Implemented as a simple linear SVM
 * trained with subgradient descent. For real-world use a native library would be preferable, but
 * this covers the ChucK AI examples which use small feature vectors.
 */
      ...
      for (int iter = 0; iter < maxIter; iter++) {
        for (int i = 0; i < n; i++) {
          ...
          if (y * score < 1.0) {
            // hinge loss gradient
            for (int j = 0; j < d; j++)
              weights[c][j] += learningRate * (y * X[i][j] - 2.0 / maxIter * weights[c][j]);
            weights[c][d] += learningRate * y;
          } else {
            for (int j = 0; j < d; j++)
              weights[c][j] -= learningRate * 2.0 / maxIter * weights[c][j];
          }
        }
      }
    }
