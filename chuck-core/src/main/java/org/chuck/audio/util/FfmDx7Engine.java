 ...
 *
 * <p>Replaces the JNI-based {@link Dx7Native} class. Calls the C API functions exported by the DLL
 * ({@code dx7_init, dx7_create_voice, ...}) directly via {@link Linker#downcallHandle}, eliminating
 * the JNI glue layer.
 *
 * <p>All voice handles are opaque {@code long} values (actually pointers) returned from {@link
 * #dx7CreateVoice()} and passed to the other methods.
 */
public final class FfmDx7Engine {
  ...
  static {
    ...
    dx7Tick =
        findAndBind(
            "dx7_tick", FunctionDescriptor.of(ValueLayout.JAVA_FLOAT, ValueLayout.JAVA_LONG));
  ...
  private static SymbolLookup loadLibrary() {
    ...
    var msg =
        "FfmDx7Engine: cannot load deluge_dsp_native.\n"
            + "  Tried libraryPath: "
            ...
            .orElseThrow(
                () ->
                    new UnsatisfiedLinkError(
                        "FfmDx7Engine: symbol '" + name + "' not found in deluge_dsp_native"));
  ...
  /** Renders nSamples into the provided float array (more efficient than per-sample tick). */
  public static void tickBlock(long handle, float[] outArray, int nSamples) {
    try (Arena arena = Arena.ofConfined()) {
      MemorySegment seg =
          arena.allocate((long) outArray.length * ValueLayout.JAVA_FLOAT.byteSize());
      ...
      MemorySegment.copy(
          ...
          0,
          MemorySegment.ofArray(outArray),
          ValueLayout.JAVA_FLOAT,
          0,
          nSamples);
    } catch (RuntimeException e) {
