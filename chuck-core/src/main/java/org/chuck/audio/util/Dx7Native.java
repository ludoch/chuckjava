 ...
 *
 * <p>Each voice is identified by an opaque {@code long handle} returned from {@link
 * #nativeCreateVoice()}. The handle wraps a C++ {@code NativeVoice} containing DxPatch + DxVoice +
 * feedback buffer + block buffer.
 *
 * <p>Calling sequence:
 *
 * <ol>
 *   <li>{@link #nativeInit()} once at startup
 *   <li>{@link #nativeCreateVoice()} for each voice needed
 *   <li>{@link #nativeLoadPatch(long, byte[])} with a 156-byte DX7 SysEx patch
 *   <li>{@link #nativeNoteOn(long, int, int)} to trigger a note
 *   <li>{@link #nativeTick(long)} or {@link #nativeTickBlock(long, float[], int)} to render
 *   <li>{@link #nativeNoteOff(long)} to release
 *   <li>{@link #nativeDestroyVoice(long)} when done
 * </ol>
 */
public final class Dx7Native {
  ...
  /**
   * Processes one sample and returns the output as a float in [-1..1]. Internally renders a block
   * of 132 samples per call and buffers the rest.
   */
  public static native float nativeTick(long handle);

  /**
   * Renders {@code nSamples} into the provided float array (more efficient than per-sample tick).
   * The array must be at least {@code nSamples} long and nSamples must be &le; 132.
   */
