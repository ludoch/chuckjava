...
/**
 * BlowHole — Clarinet model with tonehole and register vent. Extends the basic Clarinet bore model
 * with a side-hole that creates register transitions and overblown harmonics.
 */
public class BlowHole extends ChuckUGen {
  ...
  public void tonehole(double v) {
    toneCoeff = (float) (1.0 - v);
  }

  /** Register vent openness 0.0–1.0; opening shifts to upper register. */
  public void vent(double v) {
    ventCoeff = (float) v;
  }

  public void noteOn(float vel) {
    pressure = 0.4f + vel * 0.6f;
    env.keyOn();
  }

  public void noteOff(float vel) {
    env.keyOff();
  }
