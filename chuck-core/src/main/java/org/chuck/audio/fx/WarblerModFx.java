...
/**
 * WARBLER ModFX: modulated delay-tap with dual LFOs and resonance-compensated feedback.
 * Emulates the Deluge firmware's warble effect with a random-walk LFO driving delay modulation.
 */
public class WarblerModFx extends ChuckUGen {
  ...
  public void setFeedback(double fb) { this.feedback = Math.max(-0.9, Math.min(0.9, fb)); }
  ...
  protected float compute(float input, long systemTime) {
    ...
    double combinedLfo = 0.7 * Math.sin(lfo1Phase * 2.0 * Math.PI)
                       + 0.3 * warbler2;
