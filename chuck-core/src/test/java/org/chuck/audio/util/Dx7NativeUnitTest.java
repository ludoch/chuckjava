 ...
 *
 * <p>Exercises all 10 native methods declared in {@link Dx7Native}:
 * lifecycle (init, create/destroy voice), patch loading, note on/off,
 * per-sample tick, block tick, pitch bend, and isActive query.
 *
 * <p>Requires the DLL to be on java.library.path or the development
 * fallback path (deluge/dx7native/lib/). Only runs when the DLL loads
 * successfully; tests are {@code @Disabled} via assumption if loading fails.
 */
public class Dx7NativeUnitTest {
    ...
    public static void checkNativeLibrary() {
        ...
        try {
            ...
            System.err.println("Dx7Native library not available, skipping native tests: " + t.getMessage());
    ...
    public void testNoteOffAndIsActive() {
        ...
        System.out.println("testNoteOffAndIsActive: isActive after release = " + Dx7Native.nativeIsActive(handle));
