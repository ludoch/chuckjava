 ...
 *
 * <p>Exercises all 9 C API functions exposed by the DLL: init, create/destroy voice, patch loading,
 * note on/off, per-sample tick, block tick, pitch bend, and isActive query.
 *
 * <p>Requires the DLL to be found by the FFM library lookup. Tests are skipped via assumption if
 * loading fails.
 */
public class FfmDx7EngineUnitTest {
  ...
  public void testNoteOffAndIsActive() {
    ...
    System.out.println(
        "testNoteOffAndIsActive: isActive after release = " + FfmDx7Engine.isActive(handle));
