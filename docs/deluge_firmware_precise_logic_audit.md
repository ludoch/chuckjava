...
| **GAP-08** | Master delay produces no echo in the pure engine | ⛔ Open (two parts) | `FirmwareAudioEngine.java`, `PureFirmwareEngine.java`, `DelayBuffer.java` | **(a) Wiring:** `delayState.doDelay` stays `false` and `setupWorkingState` is never called before `masterDelay.process`. **(b) Deeper buffer bug (the real blocker):** even when correctly driven (`setupWorkingState` called, feedback set, rate set), the ported `DelayBuffer` resampling state machine never starts reading — `sizeLeftUntilBufferSwap` decrements a few samples then **freezes**, so `copySecondaryToPrimary` never fires, `primaryBuffer` never activates, and only the dry signal passes (verified with a standalone impulse harness: 1 non-zero output sample = the dry impulse, zero echoes). The delay-time mapping itself is solved — `userDelayRate = kNeutralSize·kMaxSampleValue / (delaySeconds·44100)` is the exact inverse of `DelayBuffer.getIdealBufferSizeFromRate`, and `G_DELAY_TIME` already carries the time in seconds — so once the buffer swap-accounting bug is fixed, wiring it is straightforward. Deferred: needs a focused debug of the `DelayBuffer` secondary-write/swap counter. |
| **GAP-09** | Bitcrush / sample-rate reduction / bass-treble EQ not implemented in the pure engine | ⛔ Open | (new DSP needed) | The C++ `ModControllableAudio::processSRRAndBitcrushing` (bit-depth reduction + decimation with interpolation) and the bass/treble EQ (`doEQ`) are part of every sound's FX chain, but the firmware Java port has **no** implementation — only the `Param` constants exist. So "gold knob" bitcrush/decimate/EQ patches lose that character entirely. Unlike GAP-06/07 this is *not* a wiring gap; it needs the DSP ported from `mod_controllable_audio.cpp` plus param wiring (rate/amount from the song), then verification. Stutter, by contrast, *is* present and wired (`Stutterer.processStutter`). |
