...
| **GAP-12** | Arpeggiator not driven in the pure engine | ⛔ Open | `FirmwareVoice.java` (+ note path) | An `Arpeggiator` is instantiated per voice but never ticked — no method on it is called, so arp patches produce no arpeggiation. This is note-level (clock-driven note on/off generation), so it belongs in the note-trigger / sequencer path, not the per-sample voice render. Needs a clock hook that feeds held notes through the arpeggiator. |
| **GAP-13** | Granular / time-stretch not wired | ⛔ Open | `VoiceSample.java`, `dsp/granular`, `dsp/timestretch` | `GranularProcessor` and `TimeStretcher` are ported but not referenced by `VoiceSample`/`FirmwareVoice`, so STRETCH/granular sample modes fall back to plain resampled playback. Mostly affects audio-clip / stretch-mode playback. |
