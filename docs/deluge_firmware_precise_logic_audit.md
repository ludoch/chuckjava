...
| **GAP-08** | Master delay disabled in the pure engine | ⛔ Open | `FirmwareAudioEngine.java`, `PureFirmwareEngine.java` | `delayState.doDelay` is set `false` at init and never updated, and `setupWorkingState` is never called before `masterDelay.process`, so the (faithfully ported) delay never runs — no echo regardless of the song's delay settings. Feedback/ping-pong/analog map cleanly, but a faithful delay *time* requires the firmware param-value system: `userDelayRate = getExp(neutralDelayRate, cableToExpParamShortcut(knob))` (`getExp` is already ported; still needs the `GLOBAL_DELAY_RATE` neutral value and the knob→internal-param mapping). Deferred rather than shipping a guessed delay time. |
