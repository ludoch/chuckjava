...
| **GAP-10** | Bass/treble EQ not implemented in the pure engine | ⛔ Open | (new DSP needed) | The C++ per-sound EQ (`doEQ` in `mod_controllable_audio.cpp`, bass/treble shelving) has no Java firmware implementation — only the `Param` constants exist. Needs the shelving-filter DSP ported plus param wiring. Stutter, by contrast, *is* present and wired (`Stutterer.processStutter`). |
