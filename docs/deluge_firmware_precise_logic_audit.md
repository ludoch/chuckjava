...
| **GAP-13b** | Sample time-stretch (STRETCH mode) not wired | ⛔ Open | `VoiceSample.java`, `dsp/timestretch` | `TimeStretcher` is ported but `VoiceSample` always resamples. Wiring it needs (a) the time-stretch ratio, which comes from the clip-length / tempo sync and isn't plumbed into `VoiceSample`, and (b) adapting the int-buffer `TimeStretcher` to the float sample pipeline. Niche (audio-clip STRETCH mode); deferred. |
