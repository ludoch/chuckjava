...
### 16.6 Delugeator Voice & Kit Generator Suite (Tools ➔ Delugeator Randomizer or Ctrl + R / Cmd + R)
To enable instantaneous, dynamic sound morphing and single-click drum kit generation, a complete JTabbedPane Generators Suite JDialog dashboard has been added:
- **Tab 1: 🎲 Synth Randomizer**: Fully replicates the official online dynamic web-randomizer rules. Utilizing continuous triangular probability distributions centered around safe default values, it morphs general parameters, state-variable subtractive wave oscillators, active dual modulations, and 6-operator DX7 FM configurations. Includes a high-contrast Hardcore mode checkbox (to bypass safe limits for extreme drive/saturation) and a custom paint-drawn dynamic color gauge pointer tracking your target average randomness live on a neon HSL linear meter.
- **Tab 2: 🥁 Kit Super-Generator**: Select any directory folder of WAV/AIF/FLAC drum samples. The tool automatically scans the folder, lists all files, and runs our dual-pass Smart Auto-Mapper engine to classify files into their standard Kit slots (Kick, Snare, Closed Hat, Open Hat, Clap, Rim, Tom, Cymbal/Ride, Percussion, etc.) using file name keyword stem checks. Includes interactive mini play audition triggers to preview selected samples, slider volume bounds, and an **Auto-Choke Hats** checkbox that automatically maps Open and Closed hats to shared Mute Group 1 for realistic choke performance. Click **[Generate & Load Kit]** to automatically write a standard Deluge Kit XML to your KITS/ folder and hot-swap your active track row to play it live instantly!

### 16.7 DAW-Grade Visual Waveform Sampler & Manual Loop Crop Deck
- **Loom-Backed Waveform Decoders**: Embedded inside our wide-screen Kit sound slot dashboard! When a drum kit WAV sample is loaded or browsed, a dedicated SwingWaveformPanel component automatically spawns a JVM virtual thread (Project Loom) to decode the raw PCM stream in parallel with zero main thread lag.
- **Teal-to-Magenta HSL Symmetric Wave Outline**: Paints a beautiful center-split visual outline representation of the audio file's transients and decay curves. The symmetric lines are drawn using a vibrant color gradient that dynamically morphs horizontally from a glowing neon teal (`#00ffcc` at the center) to a hot magenta/pink (`#ff007f` at the file bounds) over a retro oscilloscope laboratory grid backdrop.
- **Smart Path standardizer & Directory Search Fallbacks**: Built with robust directory resolver rules to locate relative paths inside standard XML files by checking global samples folders and kits folders dynamically based solely on the SD card preferences folder path. 
- **Manual loop crop sliders panel**: Features 4 visual sliders (Start, End, Loop Start, Loop End) cabled to glide the vertical neon color markers live across the waveform canvas. Clicking the **[Save & Apply Crop]** button automatically commits the positions to the kit model, exports the Kit XML, and triggers a real-time JNI playback synchronization to crop the sample loop live on ChucK's audio thread!

### 16.8 Universal Visual Audio Loop Slicer & Kit Splitter (Tools ➔ Audio Loop Slicer or Ctrl + L / Cmd + L)
To automate loop chop slice layouts across kit pads (matching the physical Deluge SLiC menu workflow), a dedicated visual Audio Slicer dialog dashboard has been added:
- **Visual orange slice-grid dividers**: Browse any breakbeat or drum loop WAV file. The waveform decodes in parallel Loom threads and instantly draws numbered vertical orange slice-dividers representing the exact mathematical loop segments (4, 8, or 16 divisions).
- **Auto-Choke and Normalization**: Automatically sets the Open and Closed kit slices to share **Mute Group 1** (so triggering a new slice cuts off the previous playing sample tail for MPC-style breakbeat grooves!) and sets standard normalized volumes.
- **One-Click Split Assembly**: Click **[Slice & Load Across Kit rows live]** to slice the breakbeat loop at accurate subdivisions, populate kit rows 0 to 15 with precise crop points, write a standard Kit XML preset to the real SD card KITS folder, and hot-swap your active sequencer track to play it live instantly!
