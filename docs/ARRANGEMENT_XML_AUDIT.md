...
| ~~`arrangementOnlyTracks`~~ | array written when present (`song.cpp:1286-1297`) | ✅ FIXED — serialized in its own block (with `trackName`), skipped from `sessionClips`, round-trips (ArrangementOnlyRoundTripTest); also fixed the buggy arrangement-only note decoder | ✅ | — |
