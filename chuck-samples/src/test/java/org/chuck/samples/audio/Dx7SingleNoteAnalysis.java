package org.chuck.samples.audio;
...
/**
 * Isolate Dx7Engine output for a single MIDI note with the Tomsweep patch. Saves WAV and runs FFT
 * to verify fundamental frequency.
 */
public class Dx7SingleNoteAnalysis {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Hardcoded real 156-byte DX7 voice patch hex
    String hex =
    ...
    System.out.println(
        "Algorithm: " + algoIdx + "  Feedback: " + feedback + "  Transpose: " + transpose);
    ...
    for (int op = 0; op < 6; op++) {
      ...
      System.out.printf(
          "  OP%d: mode=%d coarse=%d fine=%d detune=%d outlevel=%d opSw=%d%n",
    ...
    for (int op = 0; op < 6; op++) {
      ...
      System.out.printf(
          "  OP%d: dxNoteToFreq(%d) = %d (Q24) = %.3f Hz%n",
      ...
      if (mode == 1) {
        ...
        System.out.printf(
            "  OP%d Fixed: coarse=%d fine=%d logfreq=%d -> %.2f Hz%n",
            op, coarse, fine, fLogfreq, fFreq);
      } else {
        // Ratio mode — manually inline the coarsemul array
        int[] coarsemul = {
          -16777216, 0, 16777216, 26591258, 33554432, 38955489, 43368474, 47099600, 50331648,
          53182516, 55732705, 58039632, 60145690, 62083076, 63876816, 65546747, 67108864, 68576247,
          69959732, 71268397, 72509921, 73690858, 74816848, 75892776, 76922906, 77910978, 78860292,
          79773775, 80654032, 81503396, 82323963, 83117622
        };
        ...
        System.out.printf(
            "  OP%d Ratio: coarse=%d fine=%d detune=%d logfreq=%d -> %.2f Hz (base %.2f Hz, ratio %.4f)%n",
  ...
  static void testNote(byte[] patch, int midiNote) throws Exception {
    ...
    System.out.printf(
        "\nMIDI %d (expected %.2f Hz): peak=%.4f RMS=%.4f%n", midiNote, expectedFreq, peak, rms);

    // Autocorrelation-based pitch detection
    double[] freqs = detectFreqs(steady, SAMPLE_RATE);
    for (int i = 0; i < Math.min(3, freqs.length); i++) {
      System.out.printf(
          "  Freq[%d]: %.2f Hz (ratio to expected: %.3f)%n", i, freqs[i], freqs[i] / expectedFreq);
    }
    ...
    for (int div = 2; div <= 10; div++) {
      ...
      for (double f : freqs) {
        if (Math.abs(f - sub) / sub < 0.05) {
          System.out.printf(
              "  *** Subharmonic detected: expected/%.0f = %.2f Hz matches detected %.2f Hz%n",
              (double) div, sub, f);
        }
      }
    }
