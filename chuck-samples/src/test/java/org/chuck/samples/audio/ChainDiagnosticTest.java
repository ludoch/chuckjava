...
import org.chuck.samples.audio.util.AudioAnalyzer;
import org.junit.jupiter.api.Test;

/**
 * Isolate the exact kit chain in a single shred and dump samples at each stage. Chain: SndBuf →
 * ADSR → Pan2 → master(Gain) → HPF(20Hz) → Dyno(limiter) → masterTap(Gain) → WvOut2
 *
 * <p>FOCUSED DIAG: We put a WvOut2 directly after SndBuf (before ADSR) and directly after ADSR to
 * find where signal is lost during block-based advance().
 */
public class ChainDiagnosticTest {
  ...
  void diagnoseKitChain() throws Exception {
    ...
    String samplePath = "SAMPLES/DRUMS/Kick/808 Kick.wav";
    File localTarget = new File("target/test-classes/" + samplePath);
    ...
    vm.spork(
        () -> {
          ...
          System.out.println(
              "[shred] PRE-trigger: env.state=" + env.state() + " env.value=" + env.value());
          ...
          System.out.println(
              "[shred] POST-trigger: env.state=" + env.state() + " env.value=" + env.value());
          System.out.println("[shred] buf.pos=" + buf.pos() + " buf.rate=" + buf.rate());
          System.out.println(
              "[shred] buf.samples[0..4]="
                  + buf.valueAt(0)
                  + ","
                  + buf.valueAt(1)
                  + ","
          ...
          System.out.println(
              "[shred] POST-advance: env.state=" + env.state() + " env.value=" + env.value());
          ...
          System.out.println(
              "[shred] POST-diagnostic tick: buf.tick(99999)="
                  + testBufOut
                  + " env.tick(99999)="
                  + testEnvOut
                  + " env.state="
