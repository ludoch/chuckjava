...
/**
 * Minimal three-way test to isolate the engine output issue. Tests are run inside vm.spork() so
 * dac() is bound.
 */
public class SimpleCaptureTest {
  ...
  void testSndBufDirectCapture() throws Exception {
    ...
    System.out.printf(
        "CAP1 expected peak=%.6f actual=%.6f ratio=%.2f%n",
        expPeak1, peak(cap1), peak(cap1) / expPeak1);
  ...
  void testSndBufAdsrCapture() throws Exception {
    ...
    System.out.printf(
        "CAP2 expected peak=%.6f actual=%.6f ratio=%.2f%n",
        expPeak2, peak(cap2), peak(cap2) / expPeak2);
