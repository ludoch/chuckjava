...
/**
 * 4-voice Chorus effect example using the Java Fluent DSL. Four SinOsc voices at chord pitches (D
 * minor 7) run through individual Chorus units, producing a shimmering, wide stereo texture.
 *
 * <p>Original ChucK (effects/chorus.ck): SinOsc s[4]; Chorus chor[4]; [62,65,69,72] => notes; s[i]
 * => chor[i] => dac; chor[i].modDepth(.4); chor[i].modFreq(1); chor[i].mix(.2); while(true)
 * 1::second => now;
 */
