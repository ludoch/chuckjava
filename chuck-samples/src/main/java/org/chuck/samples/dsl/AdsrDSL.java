...
/**
 * ADSR Envelope example using the Java Fluent DSL. Plays 4 notes shaped by an ADSR envelope.
 *
 * <p>Original ChucK (adsr.ck): SinOsc s => ADSR e => dac; e.set(10::ms, 8::ms, .5, 500::ms); .5 =>
 * s.gain; while(true) { Math.random2(32, 96) => Std.mtof => s.freq; e.keyOn(); 500::ms => now;
 * e.keyOff(); e.releaseTime() => now; 300::ms => now; }
 */
